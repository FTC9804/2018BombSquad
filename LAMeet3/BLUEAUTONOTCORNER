...
public class BLUEAUTONOTCORNER extends FunctionsForAuto {
    ...
    public void runOpMode() throws InterruptedException { //runOpMode() method
        ...
        driveNewIMU(4.75, 10, -.25, false, 0); //Drive backwards 7.75 inches at -.25 power keeping a 0 degree heading with a 10 second limit

        pause(.8); //pause for .8 seconds

        driveNewIMU(13, 10, -.25, false, 0); //Drive backwards 10.25 inches at -.25 power keeping a 0 degree heading with a 10 second limit

        pause(.4); //pause for .4 seconds

        spinMove(178, false, 10); //Spin to 178 degrees not starting at .3 power with a 10 second timeout

        pause(.2); //pause for .45 seconds

        if (vuMarkOutput.equalsIgnoreCase("left")) //If vuforia reads left
        {
            strafeNewIMU(4, 5, .65, 178); //Strafe to the right for 10.5 inches at .63 power keeping a 178 degree heading with a 5 second limit
        }
        else if (vuMarkOutput.equalsIgnoreCase("right")) //If vuforia reads right
        {
            strafeNewIMU(17.9, 5, .65, 178); //Strafe to the right for 26.5 inches at .63 power keeping a 178 degree heading with a 5 second limit
        }
        else //Else
        {
            strafeNewIMU(12.5, 5, .65, 178); //Strafe to the right for 18.5 inches at .63 power keeping a 178 degree heading with a 5 second limit
        }
